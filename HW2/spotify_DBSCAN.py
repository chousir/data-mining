...
def plot_elbow_diagram(dataframe, columns, min_samples=5):
    """
    ...
    # 將距離排序，並繪製距離的折線圖（Elbow Diagram）
    distances = np.sort(distances[:, min_samples - 1], axis=0)
    plt.figure(figsize=(10, 6))
    plt.plot(distances)
    plt.xlabel("資料點索引")
    plt.ylabel(f"{min_samples}-th 最近鄰距離")
    plt.title("k-距離圖 (用於選擇DBSCAN的最佳eps)")
    plt.show()


def dbscan_analysis(dataframe, columns, optimal_clusters, eps=1.0, min_samples=5):
    """
    ...
    dataframe["cluster"] = dbscan.fit_predict(scaled_data)
    ...
    actual_clusters = sorted(dataframe["cluster"].unique())  # 取得實際出現的 cluster 標籤
    for cluster in actual_clusters:
        cluster_genre = dataframe[dataframe["cluster"] == cluster]["genre"]
        if len(cluster_genre) == 0:
            print(f"群 {cluster} 中無資料")
        else:
            major_genre = cluster_genre.mode()[0]
...
# plot_elbow_diagram(genre_data_unique, columns_for_clustering, min_samples=10)


## 執行 KMeans 群聚分析
optimal_clusters = 15
dbscan_analysis(genre_data_unique, columns_for_clustering, optimal_clusters, eps=1.5, min_samples=10)
