...
def plot_elbow_diagram(dataframe, columns, max_clusters=20):
    ...
    # 繪製 AIC 和 BIC 的圖來選擇最佳群數
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, max_clusters + 1), aic, label="AIC", marker="o")
    plt.plot(range(1, max_clusters + 1), bic, label="BIC", marker="o")
    plt.xlabel("群數 (Number of Clusters)")
    plt.ylabel("Information Criterion")
    plt.title("GMM Elbow Diagram (AIC and BIC)")
    plt.legend()
    plt.show()


def gmm_analysis(dataframe, columns, optimal_clusters):
    """
    ...
    dataframe["cluster"] = gmm.fit_predict(scaled_data)
...
## 執行 KMeans 群聚分析
optimal_clusters = 15
gmm_analysis(genre_data_unique, columns_for_clustering, optimal_clusters)
