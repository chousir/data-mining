...
## 建立 Random Forest 模型
lsvc = LinearSVC(random_state=42, max_iter=3000)
...
best_lsvc = LinearSVC(**best_params, random_state=42, max_iter=3000)
...
## Feature Importance
coefficients = best_lsvc.coef_

# 取絕對值再在所有類別上平均，視為該特徵對分類的重要度
...
plt.savefig("Feature_Importance_LinearSVC.png")
plt.close()
